...
[ネタバレ記事](https://zenn.dev/meimei_kr/articles/50138b90cbdde8)
...
- [x] `metadata[:width]`
- [x] 画像の配置指定方法
- [x] simple_formで`data`属性を指定する方法
- [x] RSpecで画像系のテストをする方法
- [x] `processed`について
...
Simple_formおわり。次にRSpecやって、最後にテーブル制約の件が正しいのか確認だな。

RSpec終わり！最後に、本当にテーブル制約で最小値や最大値を指定できないのかロボらんてくんに訊いてみよう。

やっぱりできないであってるらしい。一応DBレベルでかけることはできて、カスタムのSQL文をマイグレーションファイルに記述して、チェック制約（CHECK constraint）というものを使用するとのこと。ただしチェック制約はサポートしてるDBとしてないDBがあるそうだ。

よし、Rails応用7復習完了！明日は次に進む。
